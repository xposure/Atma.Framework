...
namespace Atma.Math.Types
{
    class MatrixType : AbstractType
    {
        ...
        public string ComponentWiseOperator(string op)
            => string.Format("public static {0} operator{2}({0} lhs, {0} rhs) => {0}({1});", NameThat,
        ...
        public string ComponentWiseOperatorScalar(string op, string scalarType)
            => string.Format("public static {0} operator{2}({0} lhs, {3} rhs) => {0}({1});", NameThat,
        ...
        public string ComponentWiseOperatorScalarL(string op, string scalarType)
            => string.Format("public static {0} operator{2}({3} lhs, {0} rhs) => {0}({1});", NameThat,
        ...
        public string ComponentWiseOperatorForeign(string op, string returnType)
            => string.Format("public static {3} operator{2}({0} lhs, {3} rhs) => {3}({1});", NameThat,
        ...
        public string ComponentWiseOperatorForeignL(string op, string returnType)
            => string.Format("public static {3} operator{2}({3} lhs, {0} rhs) => {3}({1});", NameThat,
                    Fields.Select(c => string.Format("lhs.{0} {1} rhs.{0}", c, op)).CommaSeparated(), op, returnType);

        public string ComponentWiseOperatorForeignScalar(string op, string scalarType, string returnType)
            => string.Format("public static {4} operator{2}({0} lhs, {3} rhs) => {4}({1});", NameThat,
        ...
        public string ComponentWiseOperatorForeignScalarL(string op, string scalarType, string returnType)
            => string.Format("public static {4} operator{2}({3} lhs, {0} rhs) => {4}({1});", NameThat,
        ...
        public string ComparisonOperator(string op)
            => string.Format("public static bool{3} operator{2}({0} lhs, {0} rhs) => bool{3}({1});", NameThat,
        ...
        public string ComparisonOperatorScalar(string op, string scalarType)
            => string.Format("public static bool{3} operator{2}({0} lhs, {4} rhs) => bool{3}({1});", NameThat,
        ...
        public string ComparisonOperatorScalarL(string op, string scalarType)
            => string.Format("public static bool{3} operator{2}({4} lhs, {0} rhs) => bool{3}({1});", NameThat,
        ...
        public override IEnumerable<Member> GenerateMembers()
        {
            ...
            //     };
            yield return new Field("values", new ArrayType(BaseType, $"[{Rows * Columns}]"))
            {
            ...
            {
                CodeString = $"new .[,] ( {Columns.ForIndexUpTo(col => "( " + Column(col).CommaSeparated() + " )").CommaSeparated()} )",
                Comment = "Creates a 2D array with all values (address: Values[x, y])"
            };

            //yield return new Property("Values1D", new ArrayType(BaseType))
            yield return new Function(new ArrayType(BaseType), "ToArray1D")
            {
                CodeString = $"new .[] ( {Fields.CommaSeparated()} )",
            ...
            foreach (var constant in BaseType.TypeConstants)
            {
                ...
                {
                    Value = Construct(this, $"{ConstTypeName}.{constant}".RepeatTimes(FieldCount)),
                    Comment = $"Predefined all-{constant} matrix"
                };

                yield return new StaticProperty("Diagonal" + constant, this)
                {
                    Value = Construct(this, Fields.Select(f => IsDiagonal(f) ? $"{ConstTypeName}.{constant}" : ZeroValue)),
        ...
        protected override IEnumerable<string> Body
        {
            get
            {
                ...
                yield return "{";
                yield return "    [Inline]get";
                yield return "    {";
                yield return $"       System.Diagnostics.Debug.Assert(fieldIndex >= 0 && fieldIndex < {Rows * Columns}, \"fieldIndex was out of range\");";
                yield return $"       unchecked {{ return values[fieldIndex]; }}";
                yield return "    }";
                yield return "    [Inline]set mut";
                yield return "    {";
                yield return $"       System.Diagnostics.Debug.Assert(fieldIndex >= 0 && fieldIndex < {Rows * Columns}, \"fieldIndex was out of range\");";
                yield return $"       unchecked {{ values[fieldIndex] = value; }}";
                yield return "    }";
                yield return "}";
                ...
                yield return "    [Inline]get";
                yield return "    {";
                yield return $"        System.Diagnostics.Debug.Assert(col >= 0 && col < {Columns}, \"col was out of range\");";
                yield return $"        System.Diagnostics.Debug.Assert(row >= 0 && row < {Rows}, \"row was out of range\");";
                yield return $"        unchecked {{ return values[col * {Rows} + row]; }}";
                yield return "    }";
                yield return "    [Inline]set mut";
                yield return "    {";
                yield return $"        System.Diagnostics.Debug.Assert(col >= 0 && col < {Columns}, \"col was out of range\");";
                yield return $"        System.Diagnostics.Debug.Assert(row >= 0 && row < {Rows}, \"row was out of range\");";
                yield return $"        unchecked {{ values[col * {Rows} + row] = value; }}";
                yield return "    }";
                yield return "}";
                ...
                yield return "public int GetHashCode()";
                yield return "{";
                ...
                yield return string.Format("public {0} Transposed => {0}({1});", ClassNameTransposed, FieldsTransposed.CommaSeparated());
                ...
                if (BaseType.HasArithmetics)
                {
                    ...
                    if (Rows == Columns)
                    {
                        ...
                        if (BaseType.IsSigned)
                        {
                            ...
                            yield return string.Format("public {0} Adjugate => {0}({1});", NameThat, FieldsTransposed.Select(f => HelperDet(HelperSubmatrix(HelperFieldsOf(Rows), ColOf(f), RowOf(f)), ColOf(f) + RowOf(f))).CommaSeparated());
                    ...
                    for (var rcols = 2; rcols <= 4; ++rcols)
                    {
                        ...
                        yield return string.Format("public static {0} operator*({1} lhs, {2} rhs) => {0}({3});",
                    ...
                    if (Columns == 4 && Rows == 4)
                    {
                        ...
                        yield return string.Format("public static {0} operator*({1} m, {2} v) => {0}({3});", BaseType.TypeName + 2, NameThat, BaseType.TypeName + 2,
                            2.ForIndexUpTo(r => cols.ForEach(c => c < 2 ? "m.m" + c + r + " * v." + "xyzw"[c] : "m.m" + c + r).Aggregated(" + ")).CommaSeparated());

                        foreach (var line in "Executes a matrix-vector-multiplication.".AsComment()) yield return line;
                        yield return string.Format("public static {0} operator*({1} m, {2} v) => {0}({3});", BaseType.TypeName + 3, NameThat, BaseType.TypeName + 3,
                            3.ForIndexUpTo(r => Columns.ForIndexUpTo(c => c < 3 ? "m.m" + c + r + " * v." + "xyzw"[c] : "m.m" + c + r).Aggregated(" + ")).CommaSeparated());
                    }

                    // matrix-vector-multiplication
                    foreach (var line in "Executes a matrix-vector-multiplication.".AsComment()) yield return line;
                    yield return string.Format("public static {0} operator*({1} m, {2} v) => {0}({3});", BaseType.TypeName + Rows, NameThat, BaseType.TypeName + Columns,
                    ...
                    yield return string.Format("public static {0} CompMul({0} A, {0} B) => {0}({1});", NameThat, Fields.Select(f => string.Format("A.{0} * B.{0}", f)).CommaSeparated());

                    foreach (var line in "Executes a component-wise / (divide).".AsComment()) yield return line;
                    yield return string.Format("public static {0} CompDiv({0} A, {0} B) => {0}({1});", NameThat, Fields.Select(f => string.Format("A.{0} / B.{0}", f)).CommaSeparated());

                    foreach (var line in "Executes a component-wise + (add).".AsComment()) yield return line;
                    yield return string.Format("public static {0} CompAdd({0} A, {0} B) => {0}({1});", NameThat, Fields.Select(f => string.Format("A.{0} + B.{0}", f)).CommaSeparated());

                    foreach (var line in "Executes a component-wise - (subtract).".AsComment()) yield return line;
                    yield return string.Format("public static {0} CompSub({0} A, {0} B) => {0}({1});", NameThat, Fields.Select(f => string.Format("A.{0} - B.{0}", f)).CommaSeparated());
                    ...
                    if (BaseType.IsFloatingPoint && Rows == 4 && Columns == 4)
                    {
                        ...
                        yield return "    System.Diagnostics.Debug.Assert(width > 0, \"width\");";
                        yield return "    System.Diagnostics.Debug.Assert(height > 0, \"height\");";
                        yield return "    System.Diagnostics.Debug.Assert(fov > 0, \"fov\");";
                        ...
                        yield return "{";
                        yield return $"    var tmp = proj * (model *  {BaseType.TypeName}4(obj, 1));";
                        ...
                        yield return "{";
                        yield return $"    var tmp =  {BaseType.TypeName + "4"}(win, 1);";
