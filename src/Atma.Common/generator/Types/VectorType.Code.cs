...
namespace Atma.Math.Types
{
    partial class VectorType
    {
        ...
        public override IEnumerable<Member> GenerateMembers()
        {
            ...
            {
                var f = ArgOf(i).ToString();
                yield return new Property(f, BaseType)
                {
                    GetterLine = $"return values[{i}];",
                    SetterLine = $"values[{i}] = value;",
                    Comment = $"{f}-component"
                };

                // f = ArgOfUpper(i).ToString();
                // yield return new Property(f, BaseType)
                // {
                //     GetterLine = $"return values[{i}];",
                //     SetterLine = $"values[{i}] = value;",
            ...
            if (Components == 2)
            {
                yield return new Property("width", BaseType)
                {
                    GetterLine = $"return values[0];",
                    SetterLine = $"values[0] = value;",
                    Comment = $"0-component"
                };
                yield return new Property("height", BaseType)
                {
                    GetterLine = $"return values[1];",
                    SetterLine = $"values[1] = value;",
                    Comment = $"1-component"
                };
            }
            // swizzle
            yield return new Field("swizzle", SwizzleType)
            {
                Readonly = true,
            ...
            if (!string.IsNullOrEmpty(BaseType.OneValue))
            {
                ...
                for (var c = 0; c < Components; ++c)
                {
                    ...
                    {
                        yield return new StaticProperty("NegativeUnit" + ArgOfUpper(c), this)
                        {
                            Value = Construct(this, c.NegativeImpulseString(BaseType.OneValue, ZeroValue, Components)),
                            Comment = $"Predefined unit-{ArgOfUpper(c)} vector"
                        };
                    }
                }
            ...
            foreach (var constant in BaseType.TypeConstants)
            {
                ...
                {
                    Value = Construct(this, $"{ConstTypeName}.{constant}".RepeatTimes(Components)),
                    Comment = $"Predefined all-{constant} vector"
                };
            }

            // values
            yield return new Function(new ArrayType(BaseType), "ToArray")
            {
                CodeString = $"new .[] ( {CompArgString} )",
            ...
            {
                ParameterString = $"{BaseTypeName}[] v",
                Code = new[] { string.Format("let c = v.Count;") },
                Initializers = Components.ForIndexUpTo(i => string.Format("(c < {0}) ? {1} : v[{0}]", i, ZeroValue)),
            ...
            // ToString
            yield return new Function(new AnyType("void"), "ToString")
            {
                Override = true,
                ParameterString = "String stringBuffer",
                CodeString = "ToString(stringBuffer, \", \")",
                Comment = "Returns a string representation of this vector using ', ' as a seperator."
            };

            var toString = new List<String>();
            toString.AddRange(Components.ForIndexUpTo(i => $"let _{ArgOf(i)} = scope String(); values[{i}].ToString(_{ArgOf(i)});"));
            toString.Add($" stringBuffer.Join(sep, {string.Join(',', Components.ForIndexUpTo(i => $"_{ArgOf(i)}"))} );");
            yield return new Function(new AnyType("void"), "ToString")
            {
                ParameterString = "String stringBuffer, String sep",
                Code = toString,
                //Code = Fields.Select(x => $"stringBuffer.Append(\"{x}\");"),
            ...
            if (!BaseType.IsBool && BaseType.Name != "double" && BaseType.Name != "long")
            {
                yield return new Indexer(BaseType)
                {
                    ParameterString = "int index",
                    Getter = new string[]{
                        $"System.Diagnostics.Debug.Assert(index >=0 && index < {Components}, \"index out of range\");",
            ...
            //     CodeString = "(lhs < rhs) ? -1 : (lhs > rhs ? 1 : 0)",
            //     Comment = "Returns -1 if lhs < rhs or 1 if lhs > rhs else 0"
            // };
            yield return new Operator(BuiltinType.TypeBool, "==")
            {
                Parameters = this.LhsRhs(),
                CodeString = Fields.Select(ComparerLsh).Aggregated(" && "),
                Comment = "Returns true if this equals rhs component-wise."
            };
            ...
            yield return new Operator(BuiltinType.TypeBool, "!=")
            {
                Parameters = this.LhsRhs(),
                CodeString = "!" + Fields.Select(ComparerLsh).Aggregated(" && "),
                Comment = "Returns true if this does not equal rhs (component-wise)."
            };
            ...
            if (BaseType.IsFloatingPoint)
            {
                ...
                yield return new ComponentWiseStaticFunction(Fields, this, "Acos", this, "v", $"System.Math.Acos({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Asin", this, "v", $"System.Math.Asin({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Atan", this, "v", $"System.Math.Atan({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Cos", this, "v", $"System.Math.Cos({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Cosh", this, "v", $"System.Math.Cosh({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Exp", this, "v", $"System.Math.Exp({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Log", this, "v", $"System.Math.Log({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Log2", this, "v", $"System.Math.Log({{0}}, 2)");
                yield return new ComponentWiseStaticFunction(Fields, this, "Log10", this, "v", $"System.Math.Log10({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Floor", this, "v", $"System.Math.Floor({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Ceiling", this, "v", $"System.Math.Ceiling({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Round", this, "v", $"System.Math.Round({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Sin", this, "v", $"System.Math.Sin({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Sinh", this, "v", $"System.Math.Sinh({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Tan", this, "v", $"System.Math.Tan({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Tanh", this, "v", $"System.Math.Tanh({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Truncate", this, "v", $"System.Math.Truncate({{0}})");
                yield return new ComponentWiseStaticFunction(Fields, this, "Fract", this, "v", $"({{0}} - System.Math.Floor({{0}}))");
                yield return new ComponentWiseStaticFunction(Fields, this, "TruncateFast", this, "v", "(int64)({0})");
            }
            ...
            // perpendicular 2d
            if (Components == 2 && BaseType.IsSigned)
            {
            ...
            else if (BaseType.IsInteger)
            {
                yield return new Function(this, "Random")
                {
                    ...
                    CodeString = Construct(this, (BaseTypeCast + "random.Next(int64.MaxValue)").RepeatTimes(Components)),
            ...
            else if (BaseType.IsFloatingPoint)
            {
                ...
                yield return new Function(this, "RandomNormal")
                {
                    Static = true,
                    ParameterString = "Random random",
                    CodeString = Construct(this, (BaseTypeCast + "(System.Math.Cos(2 * System.Math.PI_d * random.NextDouble()) * System.Math.Sqrt(-2.0 * System.Math.Log(random.NextDouble())))").RepeatTimes(Components)),
                ...
                yield return new ComponentWiseStaticFunction(Fields, this, "RandomNormal", this, "mean", this, "variance",
                    BaseTypeCast + "(System.Math.Sqrt((double){1}) * System.Math.Cos(2 * System.Math.PI_d * random.NextDouble()) * System.Math.Sqrt(-2.0 * System.Math.Log(random.NextDouble()))) + {0}")
                {
                    FirstParameter = "Random random",
