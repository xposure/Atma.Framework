    ...
    class Constructor : Member
    {
        ...
        public override IEnumerable<string> Lines
        {
            get
            {
                ...
                yield return $"{MemberPrefix} this({Parameters.CommaSeparated()})";
                ...
                if (string.IsNullOrEmpty(ConstructorChain))
                {
                    ...
                    var initializer = Fields.Select(x => (it.MoveNext() ? it.Current : Type.ZeroValue));
                    yield return $"values = .({string.Join(",", initializer)});".Indent();
