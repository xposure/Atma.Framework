...
namespace StbTrueTypeSharp
{
	public class StbTrueType
	{
		...
		public static uint stbtt__find_table(byte* data, uint fontstart, StringView tag)
		{
			...
			for (i = 0; i < (.)num_tables; ++i)
			{
				...
				var xyz = scope byte[4]*;
				xyz[0] = (data + loc + 0)[0];
				xyz[1] = (data + loc + 0)[1];
				xyz[2] = (data + loc + 0)[2];
				xyz[3] = (data + loc + 0)[3];

				let tagview = scope StringView((char8*)xyz, 4);
		...
		{
			return (uint)(((int)p[0] << 24) + ((int)p[1] << 16) + ((int)p[2] << 8) + p[3]);
		}

		public static int ttLONG(byte* p)
		{
			return (int)(((int)p[0] << 24) + ((int)p[1] << 16) + ((int)p[2] << 8) + p[3]);
		}
		...
		public static int stbtt_InitFont_internal(stbtt_fontinfo info, byte* data, int fontstart)
		{
			...
			else
			{
				...
				if ((fdarrayoff) != 0)
				{
					...
					info.fdselect =
						(stbtt__buf)(stbtt__buf_range(&b, (int)(fdselectoff), (int)((uint)b.size - fdselectoff)));
				}
		...
		public static int stbtt__GetGlyfOffset(stbtt_fontinfo info, int glyph_index)
		{
			...
			{
				g1 = (int)((uint)info.glyf + ttULONG(info.data + info.loca + glyph_index * 4));
				g2 = (int)((uint)info.glyf + ttULONG(info.data + info.loca + glyph_index * 4 + 4));
			}
		...
		public static int stbtt__run_charstring(stbtt_fontinfo info, int glyph_index, stbtt__csctx* c)
		{
			...
			int has_subrs = (int)(0);
			int clear_stack = 0;
			float* s = scope float[48]*;
			stbtt__buf* subr_stack = scope stbtt__buf[10]*;
			...
			int off_x, int off_y)
		{
			...
			int j = (int)(0);
			int i = 0;
			float* scanline_data = scope float[129]*;
			float* scanline;
			float* scanline2;
			if ((result.w) > (64))
		...
		{
			var pixels;
			byte* buffer = scope byte[8]*;
			int safe_w = (int)((uint)w - kernel_width);
			int j = 0;
			...
			for (j = (int)(0); (j) < (h); ++j)
			{
				...
				case 2:
					for (i = (int)(0); i <= safe_w; ++i)
					{
						...
						buffer[((uint)i + kernel_width) & (8 - 1)] = (byte)(pixels[i]);
		...
		public static void stbtt__v_prefilter(byte* pixels, int w, int h, int stride_in_bytes, uint kernel_width)
		{
			var pixels;
			byte* buffer = scope byte[8]*;
			int safe_h = (int)((uint)h - kernel_width);
			int j = 0;
			CRuntime.memset(buffer, (int)(0), (ulong)(8));
			for (j = (int)(0); (j) < (w); ++j)
			{
				...
				case 2:
					for (i = (int)(0); i <= safe_h; ++i)
					{
						...
						buffer[((uint)i + kernel_width) & (8 - 1)] = (byte)(pixels[i * stride_in_bytes]);
			...
			for (i = (int)(0); (i) < (num_ranges); ++i)
			{
				...
				for (j = (int)(0); (j) < (ranges[i].num_chars); ++j)
				{
					...
					else
					{
						...
						rects[k].w = ((int)(x1 - x0 + spc.padding + (int)spc.h_oversample - 1));
						rects[k].h = ((int)(y1 - y0 + spc.padding + (int)spc.v_oversample - 1));
					}
				...
				for (j = (int)(0); (j) < (ranges[i].num_chars); ++j)
				{
					...
					if ((((r.was_packed) != 0) && (r.w != 0)) && (r.h != 0))
					{
						...
						stbtt_MakeGlyphBitmapSubpixel(info, spc.pixels + r.x + r.y * spc.stride_in_bytes,
							(int)((uint)r.w - spc.h_oversample + 1), (int)((uint)r.h - spc.v_oversample + 1),
		...
		public static int stbtt__compute_crossings_x(float x, float y, int nverts, stbtt_vertex* verts)
		{
			...
			int i = 0;
			float* orig = scope float[2]*;
			float* ray = scope float[2]*;
			ray[0] = (float)(1);
			ray[1] = (float)(0);
			...
			for (i = (int)(0); (i) < (nverts); ++i)
			{
				...
				if ((verts[i].type) == (STBTT_vcurve))
				{
					...
					if ((((y) > (ay)) && ((y) < (by))) && ((x) > (ax)))
					{
						float* q0 = scope float[2]*;
						float* q1 = scope float[2]*;
						float* q2 = scope float[2]*;
						float* hits = scope float[4]*;
						q0[0] = ((float)(x0));
						q0[1] = ((float)(y0));
						q1[0] = ((float)(x1));
						q1[1] = ((float)(y1));
