...
namespace Atma
{
	...
	public static class PNG
	{
		...
		private static readonly uint8[8] header = .(137, 80, 78, 71, 13, 10, 26, 10);
		private static readonly uint[256] crcTable;
		...
		public static bool IsValid(Stream stream)
		{
			...
			// check PNG header
			let sheader = scope uint8[header.Count];

			defer stream.Seek(pos, .Absolute);

			if (stream.TryRead(.(sheader)) != .Err)
				return false;

			for (var i < header.Count)
				if (sheader[i] != header[i])
					return false;
		...
		public static bool Read(Stream stream, out int width, out int height, out Color[] pixels)
		{
			...
			let idatChunk = scope uint8[4096];
			let palette = scope uint8[0];
			let alphaPalette = scope uint8[0];
			let fourbytes = scope uint8[4];

			bool hasIHDR = false, hasPLTE = false, hasIDAT = false;

			// Check PNG Header
			Runtime.Assert(IsValid(stream), "Stream is not PNG");

			// Skip PNG header
			stream.Seek(8, .Absolute);

			// Read Chunks
			while (stream.Position < stream.Length)
			{
				let chunkStartPosition = stream.Position;

				// chunk length
				if (stream.TryRead(fourbytes) == .Err)
					return false;
				...
				if (Check("IHDR", fourbytes))
				{
					...
					// currently don't support interlacing as I'm actually not sure where the interlace step takes place
					Runtime.Assert(interlace != .Adam7, "Interlaced PNGs not implemented");

					if (depth != 1 && depth != 2 && depth != 4 && depth != 8 && depth != 16)
						Runtime.Assert(false, "depth not support.");

					Runtime.Assert(filter == 0, "filter not supported");
					Runtime.Assert(compression == 0, "compression {compression} not supported");
				}
			...
			// It would be cool to do this line-by-line so we don't need to create a buffer to store the decompressed
			// stream
			{
