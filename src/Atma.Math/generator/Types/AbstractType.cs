...
namespace Atma.Math.Types
{
    abstract class AbstractType
    {
        ...
        //TODO: Fix int.max/min
        public string ConstTypeName => BaseType.IsInteger && BaseType.Name == "int" ? "int" : BaseTypeName;
        ...
        public virtual IEnumerable<string> Constraints { get { yield break; } }

        public virtual IEnumerable<string> Attributes { get { yield break; } }
        ...
        /// </summary>
        public string Construct(AbstractType type, IEnumerable<string> args) => $" {type.NameThat}({args.CommaSeparated()})";
        /// <summary>
        /// Constructs an object of a given type
        /// </summary>
        public string Construct(AbstractType type, params string[] args) => $" {type.NameThat}({args.CommaSeparated()})";
        ...
        public IEnumerable<string> GlmSharpFile
        {
            get
            {
                ...
                // yield return "// ReSharper disable InconsistentNaming";
                // yield return "";
                yield return "namespace " + Namespace + ".glm";
                yield return "{";
        ...
        public IEnumerable<string> CSharpFile
        {
            get
            {
                ...
                //yield return "    [Ordered, Packed, CRepr]";
        ...
        public string Comparer(string val) =>
            // BaseType.Generic ?
            //     string.Format("EqualityComparer<T>.Default.Equals({0}, rhs.{0})", val) :
            //string.Format("{0}.Equals(rhs.{0})", val);
            string.Format("{0} == rhs.{0}", val);

        public string ComparerLsh(string val) =>
                    // BaseType.Generic ?
                    //     string.Format("EqualityComparer<T>.Default.Equals({0}, rhs.{0})", val) :
                    //string.Format("{0}.Equals(rhs.{0})", val);
                    string.Format("lhs.{0} == rhs.{0}", val);
        ...
        public IEnumerable<string> SwitchIndex(IEnumerable<string> cases, int components = 0, bool setter = false)
        {
            if (components > 0)
                yield return $"System.Diagnostics.Debug.Assert(index >=0 && index < {components}, \"index out of range\");";
            yield return "switch (index)";
            yield return "{";
            foreach (var @case in cases)
                yield return @case.Indent();
            if (!setter)
                yield return "    default: return default;";
            yield return "}";
        }
