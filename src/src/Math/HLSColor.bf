...
namespace Atma
{
	...
	public static class SimpleColorTransforms
	{
		...
		public static double3 RgBtoHsl(Color color)
		{
			double h = 0D;
			double s = 0D;
			double l = 0D;
		...
		public static Color HsLtoRgb(double h, double s, double l, int a = 255)
		{
			...
			{
				return .(
					(uint8)Math.Max(0, Math.Min(255, l * 255D)),
					(uint8)Math.Max(0, Math.Min(255, l * 255D)),
					(uint8)Math.Max(0, Math.Min(255, l * 255D)),
					(uint8)a);
			}
			...
			return .(
				(uint8)Math.Max(0, Math.Min(255, T[0] * 255D)),
				(uint8)Math.Max(0, Math.Min(255, T[1] * 255D)),
				(uint8)Math.Max(0, Math.Min(255, T[2] * 255D)),
				(uint8)a
				);
		}
		...
		/// </summary>
		/// <param name="color">The color to convert.</param>
		public static double3 RgBtoHsb(Color color)
		{
		...
		public static Color HsBtoRgb(double h, double s, double b, int a = 255)
		{
			...
			return .((uint8)a, (uint8)(r * 255D), (uint8)(g * 255D), (uint8)(bl * 250D));
		}
		...
		{
			double3 hsl = colorTransformMode == ColorTransformMode.Hsl
				? SimpleColorTransforms.RgBtoHsl(color)
				: SimpleColorTransforms.RgBtoHsb(color);
			if ((Math.Abs(hsl.z) < SimpleColorTransforms.tolerance)
				&& (brightnessTransform > 1D))
				hsl.z = brightnessTransform - 1D;
			else
				hsl.z *= brightnessTransform;
			return colorTransformMode == ColorTransformMode.Hsl
				? SimpleColorTransforms.HsLtoRgb(hsl.x, hsl.y, hsl.z, outputAlpha ?? color.A)
				: SimpleColorTransforms.HsBtoRgb(hsl.x, hsl.y, hsl.z, outputAlpha ?? color.A);
		}
			...
			byte? outputAlpha = null)
		{
			double3 hsl = colorTransformMode == ColorTransformMode.Hsl
				? SimpleColorTransforms.RgBtoHsl(color)
				: SimpleColorTransforms.RgBtoHsb(color);
			if ((Math.Abs(hsl.y) < SimpleColorTransforms.tolerance)
				&& (saturationTransform > 1D))
				hsl.y = saturationTransform - 1D;
			else
				hsl.y *= saturationTransform;
			if ((Math.Abs(hsl.z) < SimpleColorTransforms.tolerance)
				&& (brightnessTransform > 1D))
				hsl.z = brightnessTransform - 1D;
			else
		...
		public static Color AlphaCombine(Color color1, Color color2, byte outputAlpha)
		{
			...
			return .(
				(byte)Math.Max(0D, Math.Min(255D, ((color1.R * a1) + (color2.R * a2)) * 0.5D)),
				(byte)Math.Max(0D, Math.Min(255D, ((color1.G * a1) + (color2.G * a2)) * 0.5D)),
				(byte)Math.Max(0D, Math.Min(255D, ((color1.B * a1) + (color2.B * a2)) * 0.5D)),
				outputAlpha
				);
		}
	}
